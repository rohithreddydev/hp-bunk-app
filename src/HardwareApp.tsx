    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-4 max-w-7xl mx-auto">
        {loading ? (
          ...
          <>
            {activeTab === 'dashboard' && <HwDashboard todaySalesTotal={todaySalesTotal} todayExpenses={todayExpenses} lowStock={lowStock} topProducts={topProducts} recentSales={sales.slice(0, 8)} totalProducts={products.length} totalCreditOutstanding={totalCreditOutstanding} />}
...
// ─── Dashboard ─────────────────────────────────────────────────────────────────
function HwDashboard({ todaySalesTotal, todayExpenses, lowStock, topProducts, recentSales, totalProducts, totalCreditOutstanding }: {
...
function HwSales({ bunkId, products, customers, onRefresh, showToast }: { bunkId: string; products: Product[]; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  async function handleSell() {
    ...
    if (paymentMode === 'credit' && customerId) {
      ...
      if (cust) {
        await supabase.from('hw_customers').update({ outstanding_amount: Number(cust.outstanding_amount) + total }).eq('id', customerId);
      }
    }
...
function HwCustomers({ bunkId, customers, onRefresh, showToast }: { bunkId: string; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  async function handleCollectPayment() {
    ...
    const newOutstanding = Number(payModal.outstanding_amount) - amt;
    const { error } = await supabase.from('hw_customers').update({ outstanding_amount: newOutstanding }).eq('id', payModal.id);
