...
interface Customer {
  ...
  customer_type: string; outstanding_amount: number; is_active: boolean; created_at: string;
}
interface Sale {
...
export function StationeryApp({ bunkId }: { bunkId: string }) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-4 max-w-7xl mx-auto">
        {loading ? (
          ...
          <>
            {activeTab === 'dashboard' && <StDashboard todaySalesTotal={todaySalesTotal} todayExpenses={todayExpenses} lowStock={lowStock} totalProducts={products.length} recentSales={sales.slice(0, 8)} />}
...
function StDashboard({ todaySalesTotal, todayExpenses, lowStock, totalProducts, recentSales }: {
...
function StInventory({ bunkId, products, onRefresh, showToast }: { bunkId: string; products: Product[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  async function handleSave() {
    ...
    const { error } = editing ? await supabase.from('st_products').update(payload).eq('id', editing.id) : await supabase.from('st_products').insert(payload);
    setSaving(false);
  ...
  async function handleDelete(p: Product) {
    ...
    const { error } = await supabase.from('st_products').update({ is_active: false }).eq('id', p.id);
    if (error) { showToast(error.message, 'error'); return; }
...
interface CustForm { name: string; phone: string; address: string; customer_type: string; }
const defaultCF = (): CustForm => ({ name: '', phone: '', address: '', customer_type: 'Retail' });
...
function StCustomers({ bunkId, customers, onRefresh, showToast }: { bunkId: string; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  function openEdit(c: Customer) { setEditing(c); setForm({ name: c.name, phone: c.phone, address: c.address, customer_type: c.customer_type || 'Retail' }); setShowModal(true); }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-xs uppercase">
              <tr><th className="px-4 py-3 text-left">Name</th><th className="px-4 py-3 text-left">Type</th><th className="px-4 py-3 text-left">Phone</th><th className="px-4 py-3 text-left">Address</th><th className="px-4 py-3 text-right">Outstanding</th><th className="px-4 py-3 text-center">Actions</th></tr>
            </thead>
            <tbody>
              ...
              {filtered.map(c => (
                <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center"><button onClick={() => openEdit(c)} className="text-amber-600 hover:text-amber-800"><Edit2 size={14} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
