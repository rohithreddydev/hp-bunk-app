...
// ─── Main Component ────────────────────────────────────────────────────────────
export function AutoPartsApp({ bunkId }: { bunkId: string }) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-4 max-w-7xl mx-auto">
        {loading ? (
          ...
          <>
            {activeTab === 'dashboard' && <ApDashboard todaySalesTotal={todaySalesTotal} todaySalesCount={todaySales.length} todayExpenses={todayExpenses} lowStock={lowStock} recentSales={sales.slice(0, 8)} />}
...
// ─── Dashboard ─────────────────────────────────────────────────────────────────
function ApDashboard({ todaySalesTotal, todaySalesCount, todayExpenses, lowStock, recentSales }: {
...
function ApSales({ bunkId, products, customers, onRefresh, showToast }: { bunkId: string; products: Product[]; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  async function handleSell() {
    ...
    setCart([]); setCustomerId(''); setVehicleNumber(''); setLabourCharges(0); setNotes('');
...
// ─── Customers ─────────────────────────────────────────────────────────────────
interface ACustForm { name: string; phone: string; address: string; vehicle_number: string; vehicle_model: string; }
const defaultACF = (): ACustForm => ({ name: '', phone: '', address: '', vehicle_number: '', vehicle_model: '' });
...
function ApCustomers({ bunkId, customers, onRefresh, showToast }: { bunkId: string; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  function openEdit(c: Customer) { setEditing(c); setForm({ name: c.name, phone: c.phone, address: c.address, vehicle_number: c.vehicle_number || '', vehicle_model: c.vehicle_model || '' }); setShowModal(true); }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-xs uppercase">
              <tr><th className="px-4 py-3 text-left">Name</th><th className="px-4 py-3 text-left">Phone</th><th className="px-4 py-3 text-left">Vehicle</th><th className="px-4 py-3 text-right">Outstanding</th><th className="px-4 py-3 text-center">Actions</th></tr>
            </thead>
            <tbody>
              ...
              {filtered.map(c => (
                <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center"><button onClick={() => openEdit(c)} className="text-slate-600 hover:text-slate-800"><Edit2 size={14} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
