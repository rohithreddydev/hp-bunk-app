...
interface Customer {
  ...
  land_area: string; credit_limit: number; outstanding_amount: number; is_active: boolean; created_at: string;
}
interface Sale {
...
export function AgricultureApp({ bunkId, onLogout, user }: { bunkId: string; onLogout: () => void; user: { name: string; email: string; role: string } }) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-4 max-w-7xl mx-auto">
        {loading ? (
          ...
          <>
            {activeTab === 'dashboard' && <AgDashboard todaySalesTotal={todaySalesTotal} todayExpenses={todayExpenses} lowStock={lowStock} totalProducts={products.length} recentSales={sales.slice(0, 8)} totalCreditOutstanding={totalCreditOutstanding} />}
...
function AgDashboard({ todaySalesTotal, todayExpenses, lowStock, totalProducts, recentSales, totalCreditOutstanding }: {
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {[
            ...
            { done: lowStock.length === 0, text: `Reorder low stock items${lowStock.length > 0 ? ` (${lowStock.length} items)` : ''}` },
            { done: totalCreditOutstanding === 0, text: `Collect farmer credit${totalCreditOutstanding > 0 ? ` (${inr(totalCreditOutstanding)} due)` : ''}` },
...
function AgSales({ bunkId, products, customers, onRefresh, showToast }: { bunkId: string; products: Product[]; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  async function handleSell() {
    ...
    if (paymentMode === 'credit' && customerId) {
      ...
      if (cust) {
        await supabase.from('ag_customers').update({ outstanding_amount: Number(cust.outstanding_amount) + total }).eq('id', customerId);
      }
    }
...
interface CustForm { name: string; phone: string; address: string; land_area: string; credit_limit: number; }
const defaultCF = (): CustForm => ({ name: '', phone: '', address: '', land_area: '', credit_limit: 0 });
...
function AgCustomers({ bunkId, customers, onRefresh, showToast }: { bunkId: string; customers: Customer[]; onRefresh: () => void; showToast: (m: string, t?: 'success' | 'error') => void; }) {
  ...
  function openEdit(c: Customer) { setEditing(c); setForm({ name: c.name, phone: c.phone, address: c.address, land_area: c.land_area || '', credit_limit: c.credit_limit || 0 }); setShowModal(true); }
  ...
  async function handleCollectPayment() {
    ...
    const { error } = await supabase.from('ag_customers').update({ outstanding_amount: Number(payModal.outstanding_amount) - amt }).eq('id', payModal.id);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 text-xs uppercase">
              <tr><th className="px-4 py-3 text-left">Name</th><th className="px-4 py-3 text-left">Phone</th><th className="px-4 py-3 text-left">Land Area</th><th className="px-4 py-3 text-right">Credit Limit</th><th className="px-4 py-3 text-right">Outstanding</th><th className="px-4 py-3 text-center">Actions</th></tr>
            </thead>
            <tbody>
                ...
                <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button onClick={() => openEdit(c)} className="text-green-600 hover:text-green-800"><Edit2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <div className="p-5 space-y-3">
              ...
              <div><label className="text-xs text-gray-500 mb-1 block">Credit Limit (₹)</label><input type="number" value={form.credit_limit} onChange={e => setForm(f => ({ ...f, credit_limit: parseFloat(e.target.value) || 0 }))} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" /></div>
            </div>
            <div className="flex gap-3 p-5 border-t">
              <button onClick={() => setShowModal(false)} className="flex-1 border border-gray-300 text-gray-700 py-2 rounded-lg text-sm font-medium hover:bg-gray-50">Cancel</button>
      ...
      {payModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-3">
              <p className="text-sm text-gray-600">Farmer: <span className="font-semibold text-gray-800">{payModal.name}</span></p>
              ...
              <div><label className="text-xs text-gray-500 mb-1 block">Amount Received (₹) *</label><input type="number" value={payAmount} onChange={e => setPayAmount(e.target.value)} placeholder="0" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" /></div>
